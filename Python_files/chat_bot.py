...
summarizer_prompt = ChatPromptTemplate.from_messages(
    [
        SystemMessage(
            content="""Тебе на вход даются жалобы и симптомы пациента. 
            Твоя задача кратко, но сохраня все фактические детали, проссумировать переданную пациентом информацию
            о его состоянии. Текст должен быть сжатым. Пиши от неопредлённого лица. 
            Не используй в тексте слова "пациент" или "у вас". Например, вместо "Пациент жалуется на трёхдневную боль в горле" или
            "У вас три дня болит горло", напиши "Три дня боль в горле." 
            Не указывай возможные причины. Не указывай рекомендации. Только информацию, содержащуюся в исходном тексте.
            """
        ),  
        MessagesPlaceholder(
            variable_name="chat_history"
